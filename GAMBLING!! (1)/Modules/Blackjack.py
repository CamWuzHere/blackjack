...
def game():
    ...
    coins = {f'{BOLD}{YELLOW}⛁{RESET}': 5, f'{BOLD}{YELLOW}⛀{RESET}': 1}
    player_coins = 100
    ...
    while True:
        if player_coins <= 0:
            ...
            typewriter("......",0.25)
            time.sleep(0.5)
            typewriter(f"I have college funds, right???{RESET}",0.05)
            funds = input("",end='')
            funds = funds.lower()
            if funds == 'yes' or 'yea' or 'yeah':
                print("hell yeah.") #How would I make it so it will start with the player
                game()  # able to bet an amount that I pick, and make a value called 'gambledfunds' to be true?
